...
- **Prompt parsing** — A hand-tuned keyword lexicon (`src/audio/promptParser.ts`) maps descriptive words ("dark", "punchy", "long-tailed", "clipped"...) to synthesis parameter deltas. Inspectable and predictable rather than a black box.
...
- **Variations & identity** — Three variations come from one seeded "recipe" per sound (`src/audio/variationEngine.ts`). Selecting one and adjusting Attack/Decay/Punch/Tone/Distortion re-renders that same seed with new parameters, so the sound's character stays put. Hitting "Regenerate" creates a genuinely new seed and a new set of three.
